...
public class Loader {
	...
	public RawModel loadToVAO(float[] positions,
			float[] texturedCoords,float[] normals , int[] indices){
		int vaoID = createVAO();
		int indicesVboID = bindIndicesBuffer(indices);
		storeDataInAtrributeList(0,3, positions);
		storeDataInAtrributeList(1, 2, texturedCoords);
		storeDataInAtrributeList(2, 3, normals);

		unbindVAO();
		return new RawModel(vaoID,indices.length,indicesVboID);
		
	}
	public RawModel loadToVAO(float[] positions , int dimension){
		int vaoID = createVAO();
		this.storeDataInAtrributeList(0, dimension, positions);
		unbindVAO();
		return new RawModel(vaoID, positions.length/dimension, 0);
	}
...
//				float amount = Math.min(4f, GL11.glGetFloat(EXTTextureFilterAnisotropic.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT));//higher mean higher quality;
